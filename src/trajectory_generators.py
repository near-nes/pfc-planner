...
class MLPTrajectoryGenerator(TrajectoryGeneratorBase):
...
class MLPTrajectoryNet(nn.Module):
...
def get_trajectory_generator(
    ...
    Args:
        generator_type: Type of generator ('minjerk' or 'nn')
        params: PlannerParams configuration
        model_path: Path to pretrained NN model (only for 'nn' type)
    ...
    if generator_type == "minjerk":
        ...
        return MLPTrajectoryGenerator(params, model_path)
    elif generator_type == "gle":
        return GLETrajectoryGenerator(params)
    else:
        raise ValueError(f"Unknown trajectory generator type: {generator_type}")
