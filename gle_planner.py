...
if __name__ == "__main__":
    ...
    UPDATE_STEPS = (
        10  # Number of updates per batch, can be adjusted based on training needs
    )
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, true_trajectory, target_choice_idx) in enumerate(train_loader):
            ...
            target = torch.cat(
                (
                    ...
                    torch.nn.functional.one_hot(
                        target_choice_idx, num_classes=num_choices
                    ),
                ),
                dim=1,
            )
            ...
            with torch.no_grad():
                for _ in range(
                    UPDATE_STEPS
                ):  # Assuming UPDATE_STEPS is defined as the number of updates per batch
                    output = model(
                        images, target, beta=1.0
                    )  # Assuming beta is a hyperparameter for the error computation
                optimizer.step()
            ...
            loss_trajectory = criterion_trajectory(
    ...
    # --- Save the model ---
    MODEL_SAVE_PATH = (
        "./models/trained_gle_planner.pth"  # Choose a meaningful file name
    )
